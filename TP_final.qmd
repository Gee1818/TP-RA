...
Para la variable target `tasa_grad`, los coeficientes de person mas relevantes se dan para la siguientes variables,

1. `cuota` con un valor positivo de 0.57
1. `top10` con un valor positivo de 0.5
1. `prof_dr` con un valor positivo de 0.31
1. `razon` con un valor negativo de 0.31
...
X_train = df_train %>% 
  dplyr::select(predictors(sel_mod)) %>% 
  as.matrix()
...
mod_ridge = train(
  x = X_train, y= Y_train,
  method = "glmnet",
  tuneGrid = expand.grid(alpha = 0, lambda = seq(0,1,by = 0.2)),
  metric = "RMSE",
...
```{r}
ggplot(df, aes(x = tasa_grad_binaria, fill = tasa_grad_binaria))+
  geom_bar() + 
  labs(title = "Cantidad de alumnos por tasa de graduados (binaria)",
       x = "",
       y = "Cant. alumnos")+
  theme_minimal()
```
